...
[![GitHub release (with filter)](https://img.shields.io/github/v/release/BeiWei33/yysDesktop)](https://github.com/BeiWei33/yysDesktop/releases/latest)

## 简介

本项目仅支持阴阳师桌面版使用，支持前台交互和后台交互模式。程序名称为 `yysDesktop`。

### 分辨率与窗口支持

程序会将游戏客户区统一归一化为 `1136 X 640` 标准画面，再进行截图、识别和输入坐标换算。Windows 100%、125%、150% 等显示缩放，以及可变大小的 16:9 游戏窗口均可使用；不需要再通过全局 Windows 缩放设置修复识别。截图中的有效游戏内容区与 Win32 后台输入客户区会分别校准，避免黑边或 DPI 缩放导致点击位置偏移。

非 16:9 的有效游戏内容会停止处理并提示检查游戏窗口。窗口管理中的预览和任务截图均使用 `1136 X 640` 标准画面。
...
###  2. 运行本软件
  ...
  1. 前往 [releases](https://github.com/BeiWei33/yysDesktop/releases/latest)
  2. 下载最新压缩包 `yysDesktop-2.x.x.zip`
  3. 解压到英文路径，双击 `yysDesktop.exe` 运行。
...
1. 下载源码  
    ...
    ```bash
    git clone https://github.com/BeiWei33/yysDesktop.git
    ```
