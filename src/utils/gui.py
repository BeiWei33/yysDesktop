...
class MainWindow(FluentWindow):
    ...
    def _init_settings(self):
        ...
        card.input_motion_card.switch.setChecked(setting.input_motion_enabled)
    ...
    def preview_window(self):
        ...
        if data and int(data):
            ...
            try:
                ...
                if image is None:
                    raise CaptureUnavailableError(handle, 1, "capture returned no normalized image")
                ...
                widget.update_capture_size_label(
                    f"标准画面 {CANONICAL_SIZE[0]} X {CANONICAL_SIZE[1]}"
                )
                widget.update_capture_time_label(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]}")
            except (ViewportDetectionError, CaptureUnavailableError) as error:
