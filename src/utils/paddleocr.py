...
def get_ocrdata_from_result(raw_result, offset: tuple[int, int] = (0, 0)):
    ...
    for text, score, box in zip(rec_texts, rec_scores, rec_boxes):
        item = {
            ...
            "Score": score,
            "BoxPoints": [
                {"X": int(box[0]) + offset_x, "Y": int(box[1]) + offset_y},
                {"X": int(box[2]) + offset_x, "Y": int(box[1]) + offset_y},
                {"X": int(box[2]) + offset_x, "Y": int(box[3]) + offset_y},
                {"X": int(box[0]) + offset_x, "Y": int(box[3]) + offset_y},
            ],
        }
...
class OcrData:
    ...
    def __init__(self, item: dict) -> None:
        ...
        center = self.rect.get_center_point()
        self.center = Point(
            center.client_x,
            center.client_y,
            click_bounds=(self.x1, self.y1, self.x2, self.y2),
        )
...
class OcrDetector:
    ...
    def __init__(self, region: tuple | None = None):
        ...
        """
        if region is None or region == (0, 0, 0, 0):
            region = (0, 0, *CANONICAL_SIZE)
        self.region = region
    ...
    def get_raw_result(self) -> list[OcrData]:
        ...
        try:
            ...
            for res in ocr_result:
                formatted_result.extend(get_ocrdata_from_result(res, offset=self.region[:2]))
        except Exception as e:
...
class RuleOcr:
    ...
    def __init__(
        ...
        if self.region is None or self.region == (0, 0, 0, 0):
            self.region = (0, 0, *CANONICAL_SIZE)
