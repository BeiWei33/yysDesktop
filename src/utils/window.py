...
class GameWindow:
    ...
    def __init__(self, handle: int):
        ...
        self.refresh_client_geometry()
        self._geometry_refresh_enabled = True

    def refresh_client_geometry(self) -> tuple[int, int, int, int]:
        """Refresh client dimensions and its screen-space origin from Win32."""
        ...
        self.client_top_left = win32gui.ClientToScreen(self.handle, (0, 0))
        self.client_left = self.client_top_left[0]
        self.client_top = self.client_top_left[1]
        self.screen_client_rect = (
            self.client_left,
            self.client_top,
            self.client_width,
            self.client_height,
        )
        return self.screen_client_rect
...
class GameWindowManager:
    ...
    def _update(self, window: GameWindow):
        ...
        if (
            ...
            and getattr(previous, "screen_client_rect", None)
            != getattr(window, "screen_client_rect", None)
        ):
            viewport_registry.invalidate(window.handle)
        self.current = window
    ...
    def update_window_task(self):
        ...
        if target_handles != self.handles:
            ...
            for removed_handle in set(self.handles) - set(target_handles):
                viewport_registry.invalidate(removed_handle)
